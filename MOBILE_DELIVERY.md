...
| `DEV_API_URL` | `http://localhost:8080` | API the dev build points at |
| `PROD_API_URL` | `https://api.freesolo.osas.cloud` | API the release build points at |
...
| `ANDROID_PACKAGE_NAME` | `com.osascloud.freesolo` | Android application ID |
| `IOS_BUNDLE_IDENTIFIER` | `com.freesolo.app` | iOS bundle ID |
...
1. Firebase console → add an Android app with package `com.osascloud.freesolo`
   (and an iOS app with bundle ID `com.freesolo.app` if you enable the iOS lane).
...
| Symptom | Cause |
| --- | --- |
| `Could not find scheme FreeSolo` | `expo prebuild` names the Xcode project after the app name. If the app is renamed, set the `IOS_SCHEME` variable — no code change needed |
